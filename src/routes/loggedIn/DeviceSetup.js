...
import { getBluetoothStatusOnOff, listenToDeviceScans, watchSdkEmitter } from '../../components/native';
...
import { View, Text, Image, FlatList, TouchableOpacity, StyleSheet, Pressable, Alert, Modal, Linking, Platform } from 'react-native';
...
const DeviceSetup = ({ navigation }) => {
  ...
  const [scanStatus, setScanStatus] = useState({ isScanned: false, isScanning: false })

  const filters = ['All', 'Smart Watch', 'Tracker'];

  useEffect(() => {
    const interval = setInterval(() => {
      getBluetoothStatusOnOff().then(status => {
    ...
    <View style={styles.main}>
      <View style={styles.topBar}>
        ...
        <Pressable hitSlop={20} onPress={() => navigation.goBack()}>
          <Image source={AppImages.decline} style={styles.decline_img} resizeMode="contain" />
        </Pressable>
      </View>
      ...
      <View style={styles.filterContainer}>
        {filters.map(filter => (
          <Pressable
            ...
            <Text style={[styles.filterText, selectedFilter === filter && styles.selectedText]}>{filter}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <FlatList
        ...
        ListFooterComponent={() => {
          return (
            (!(scanStatus?.isScanned == false && scanStatus?.isScanning == false) && scanStatus?.isScanning) && <FastImage source={AppImages.loading} style={styles.loading} resizeMode='contain' />
          )
        }}
      />

      {scanStatus?.isScanned && <CustomBtn
        btnName={'Scan again'}
        textStyle={{ color: baseColors.white, fontFamily: AppFonts.bold, fontWeight: '600' }}
        btnStyle={{ marginVertical: hp(1), backgroundColor: baseColors.theme, borderColor: baseColors.theme, borderWidth: 1, marginBottom: hp(0.5) }}
        onPress={() => {
          onScan()
        }}
      />}
      ...
      {/* <CustomBtn
        ...
        textStyle={{ color: baseColors.white, fontFamily: AppFonts.bold, fontWeight: '600' }}
        btnStyle={{ marginVertical: hp(1), backgroundColor: baseColors.theme, borderColor: baseColors.theme, borderWidth: 1, marginBottom: hp(2) }}
        onPress={() => {
