...
const BetDetail = ({ targetSteps = 10000, calories = 250 }) => {
  ...
  const data = {
    ...
    about: [
      ...
      // {
      //   title: '📝 Key Notes:',
      //   description: 'Weekly reset, make every step count! ',
      //   icon: '',
      // },
    ],
    joinPoints: 49,
  ...
  async function JoinBet(bet_id,) {
    ...
    try {
      ...
      dispatch(setLoader(false));
      if (res?.status == 201) {
        AppUtils.showToast('Successfully joined the pool!');
        // GetRecommended_bets();
        fetchBetDetails()
      } else {
          ...
          <View style={styles.statsSection}>
            ...
            <View style={styles.statItem}>
              ...
              {betDetails?.bet_type == "mega_pool" ?
                <Text style={styles.statValue}>{` ${parseInt(betDetails?.pool_prize) ?? ''}`} Points</Text>
                : <Text style={styles.statValue}>{` ${priceDetail?.winningAmount ?? ''}`} Points</Text>
              }
            </View>
            <Separator />
            ...
            <View style={styles.statItem}>
              ...
              <Text
                ...
                }>{betDetails?.bet_type == 'mega_pool' ? (parseInt(betDetails?.pool_prize) / 2 ?? 0) : ((priceDetail?.winningAmount) ?? 0)} Points</Text>
            </View>
          </View>
      ...
      {!betDetails?.is_joined ? (
        ...
        <CustomBtn
          ...
          onPress={() => {
            onPressTerms()
            return
            Linking.openURL('https://tychee.in/f/terms-and-conditions')
          }}
        />
      )}
    </View>
