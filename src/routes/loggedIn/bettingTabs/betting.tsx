...
import { getAll_bets, getMyBets, joinBet } from '../../../api/Services/services';
...
import { Get_user_detail, Get_wallet, setAuthRedux, setIsConnected, setToken } from '../../../redux/Reducers/userData';
...
function Dashboard(props: { setDetailType: (type: 'pickOfDay') => void }) {
  const { deviceAddress, auth, user, token, deviceActivity, isSkipped } = useSelector(state => state?.userData);
  ...
  const CheckConnectionStatus = async () => {
    isDeviceConnected()
      .then((i) => {
        console.log('connection statuse-=-=',i)
        ...
        if (!i) {
          ...
          connectBT().then((i) => console.log('99'))
        }
      })
      .catch((error) => {
  ...
  const connectWatch = async () => {
    try {
      await selectBleDevice(userRef?.current?.device_id);
        ...
        <View style={styles.cardContent}>
          ...
          {/* Prize */}
          {
            betType == 'floating_pool' ?
              <Text style={styles.cardPrize}>Prize: {(totalAmount - deduction) ?? ''}</Text>
      ...
      <Pressable style={styles.activeBetsCardContainer} onPress={onPress}>
        ...
        <View style={styles.activeBetContent}>
          <View style={styles.leftContent}>
            ...
            {
              betType == 'floating_pool' ?
                <Text style={styles.activeBetPrize}>Prize: {(totalAmount - deduction) ?? ''} Points</Text>
                : <Text style={styles.activeBetPrize}>Prize: {(prize) ?? ''} Points</Text>
            }
        ...
        <View style={styles.prizesRow}>
          <View style={styles.prizeItem}>
            ...
            {betType == 'megapool' && (
              <>
                <Text style={styles.prizeHeading}>Pool Prize</Text>
                <Text style={styles.prizeValue}>{pool_prize ?? 0} points</Text>
              </>
            )}
          </View>
          <Separator />
          {betType == 'megapool' && (
            <>
              <View style={styles.prizeItem}>
                ...
                <Text style={styles.firstPrizeValue}>{(parseInt(pool_prize ?? 0) / 2) ?? ''}</Text>
              </View>
              <Separator />
            </>
          )}
    ...
    <>
      ...
      >
        ...
        {topActiveBets.length > 0 && !isSkipped && (
          <>
            <BettingDashboardHeading title="Active Pools" />
