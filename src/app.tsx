...
import { ActivityIndicator, LogBox, Platform, useColorScheme } from 'react-native';
...
export function App(): React.JSX.Element {
  ...
  const checkSignIn = useCallback(async () => {
    ...
    // TODO handle the authentication logic here
  ...
  async function onMessageReceived(message) {
    console.log('message message message aaya====>', JSON.stringify(message))
      ...
      <PersistGate loading={null} persistor={persistor}>
        <ThemeProvider value={colorScheme === 'dark' ? DarkTheme : DefaultTheme}>
          <RecoilRoot initializeState={({ set }) => set(authAtom, initRes)}>
            <MagnusThemeProvider theme={colorScheme === 'dark' ? DarkTheme : DefaultTheme}>
              <Router />
            </MagnusThemeProvider>
          </RecoilRoot>
        </ThemeProvider>
