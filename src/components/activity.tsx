...
import { UploadDeviceActivity, setDeviceActivity } from '../redux/Reducers/userData';
import { RootState } from '../redux/store';
import { GoalFireIcon, GoalWalkingIcon } from '../routes/loggedIn/bettingTabs/icons';
...
export function Label(props: { Icon: any; value: string | number; label: string }) {
  return (
...
export function Activity({ token, deviceAddress }) {
  ...
  const [activityData, setActivityData] = useState<StepData>({ step: 0, distance: 0, calories: 0 });
  ...
  const fetchUserActivity = useCallback(async () => {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        const watchSteps = activityData.step !== -1 ? activityData.step : response.data.data.steps;
        const data = {
    ...
    if (deviceAddress) {
      watchSdkEmitter.addListener('stepChanged', async data => {
        ...
        if (data?.step) {
          ...
          setActivityData({...data,calories:convertDecaCaloriesToWatchFormat(data?.calories)})
          if ((currentStepDataRef.current?.step == data?.step) && isSameDate(data?.date, currentStepDataRef.current?.date)) {
            return
          }
  ...
  const saveActivityData = async (data: StepData) => {
    if (!token || !deviceAddress || !userId) {
      console.log('Skipping save - missing required data:', { token, deviceAddress, userId });
      return;
    }
  ...
  return (
    <View>
      <Div row mt={25} justifyContent="space-around">
        ...
        <Label Icon={GoalFireIcon} label="Calories" value={activityData.calories} />
      </Div>
    </View>
  );
}
